...
class Openpose(object):
    ...
    def save_state(self, val_loss, to_save_folder=False, model_only=False):
        ...
        if not model_only:
            ...
            torch.save(
                self.model.state_dict(), os.path.join(save_path,
                                                      'optimizer_{}_val_loss:{}_step:{}.pth'.format(time, val_loss,
                                                                                                    self.step)))
                ...
                if (self.step % self.board_pred_image_every == 0) & (self.step != 0):
                    ...
                    with torch.no_grad():
                        for i in range(20):
                            ...
                            img_path = os.path.join(params['coco_dir'], 'val2017',
                                                    self.val_loader.dataset.coco.loadImgs([img_id])[0]['file_name'])
        ...
        for i, (limb, color) in enumerate(zip(params['limbs_point'], limb_colors)):
            if i != 9 and i != 13:  # don't show ear-shoulder connection
                ...
                if np.all(pose[limb_ind][:, 2] != 0):
                    ...
                    cv2.line(canvas, tuple(joint1), tuple(joint2), color, 6)
