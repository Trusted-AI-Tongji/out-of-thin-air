...
class Openpose(object):
    ...
    def train(self, resume=False):
        ...
        x_move_factor = 100  # 横坐标噪声移动的调节参数
        y_move_factor = -150  # 纵坐标噪声移动的调节参数
        ...
        for ((imgs, pafs, heatmaps, ignore_mask), xyz) in zip(tqdm(iter(self.train_loader)), range(0, 5000)):
            ...
            per_max_change_above = 200
            per_max_change_below = 0
            ...
            imgs_perturbation_pre = Variable(torch.zeros(imgs[0].shape), requires_grad=True).cuda()
            ...
            for i in range(0, epochs):
                ...
                total_loss_1 = total_loss.data.cpu()
                pafs_loss_1 = pafs_loss.data.cpu()
                heatmaps_loss_1 = heatmaps_loss.data.cpu()
                
                '''deepfool_1攻击算法'''
                w = imgs.grad.data.cpu().numpy().copy()
                f = heatmaps_loss.data.cpu().numpy()
                print(heatmaps_loss_1)
                ...
                imgs_perturbation = (1 + overshoot) * torch.from_numpy(r_tot).to(self.device)  # 保存对抗扰动数据
                # imgs_perturbation_pre = imgs_perturbation
                if total_loss_1 > 200:
                    break
                ...
                对抗扰动
                '''
                imgs_perturbation = imgs_perturbation.cuda().data.cpu().numpy()[0]  # 四维变三维
                img_cache = imgs.cuda().data.cpu().numpy()[0]  # 四维变三维
                adv_cache = adv_cache.cuda().data.cpu().numpy()[0]  # 四维变三维
                
                # '''PIL裁剪扰动'''
                keypoints_x_min_second, keypoints_y_min_second, keypoints_x_max, keypoints_y_max = 486, 253, 590, 372
                for i_height in range(736):  # 遍历图片的所有像素
                    for j_width in range(736):
                        ...
                        and i_height > (keypoints_y_min_second - clip_factor) and i_height < (keypoints_y_max + clip_factor):
                            adv_cache[0, i_height + y_move_factor, j_width + x_move_factor] = img_cache[0, i_height + y_move_factor, j_width + x_move_factor] + imgs_perturbation[0, i_height, j_width]  # 将img1的图像该位置像素替换成img2
                            adv_cache[1, i_height + y_move_factor, j_width + x_move_factor] = img_cache[1, i_height + y_move_factor, j_width + x_move_factor] + imgs_perturbation[1, i_height, j_width]
                            adv_cache[2, i_height + y_move_factor, j_width + x_move_factor] = img_cache[2, i_height + y_move_factor, j_width + x_move_factor] + imgs_perturbation[2, i_height, j_width]
                '''
                局部对抗样本保存
                '''
                adv_cache = adv_cache.transpose(1, 2, 0)
                adv_cache = (adv_cache * std) + mean
                adv_cache = adv_cache * 255.0
                # adv_cache = np.clip(adv_cache, max_change_below, max_change_above)
                adv_cache = np.clip(adv_cache, 0, 255).astype(np.uint8)

                im = Image.fromarray(adv_cache)
                img_attacked_path = 'output/poseflow/adv_{}.jpg'.format(2)
                im.save(img_attacked_path)
                
                
                adv_orig = cv2.imread(img_attacked_path)[..., ::-1]
                im = adv_orig.copy().astype(np.float32)
                # im = cv2.resize(im, (736, 736), interpolation=cv2.INTER_CUBIC)
                im /= 255.0
                im = (im - mean) / std
                im = im.transpose(2, 0, 1)
                im = np.expand_dims(im, axis=0)
                adv_cache = Variable(torch.from_numpy(im).cuda().float(), requires_grad=True)
        
        
                '''
                对抗扰动保存
                '''
                imgs_perturbation = imgs_perturbation.transpose(1, 2, 0)
                imgs_perturbation = (imgs_perturbation * std) + mean
                imgs_perturbation = imgs_perturbation * 255.0
                # imgs_perturbation = np.clip(imgs_perturbation, per_max_change_below, per_max_change_above)
                # # adv = adv[..., ::-1]  # RGB to BGR
                imgs_perturbation = np.clip(imgs_perturbation, 0, 255).astype(np.uint8)
                im = Image.fromarray(imgs_perturbation)
                # im = im.resize((image_width, image_height), Image.BICUBIC)
                img_attacked_path = 'output/perturbation_{}.jpg'.format(per_count)
                im.save(img_attacked_path)
                per_count += 1
